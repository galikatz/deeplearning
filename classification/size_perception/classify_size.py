...
for image_path in test_image_path:
	...
	(fewLabel, manyLabel) = model.predict(image)[0]

	# build the label
	label = "few" if fewLabel > manyLabel else "many"
	proba = fewLabel if fewLabel > manyLabel else manyLabel
	...
	cv2.putText(output, label, (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,
		0.7, (0, 255, 0), 2)
	file_name = 'Output_size_class_' + label.replace(' ', '_').replace('%', '').replace(':', '')
